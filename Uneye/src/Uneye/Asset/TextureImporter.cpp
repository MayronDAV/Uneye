...
namespace Uneye
{
	Ref<Texture2D> TextureImporter::ImportTexture2D(AssetHandle p_handle, const AssetMetadata& p_metadata)
	{
		...
		return LoadTexture2D(Project::GetActiveAssetDirectory() / p_metadata.FilePath);
	}

	Ref<Texture2D> TextureImporter::LoadTexture2D(const std::filesystem::path& p_path)
	{
		UNEYE_PROFILE_FUNCTION();
		...
		{
			UNEYE_CORE_ERROR("TextureImporter::ImportTexture2D - Could not load texture from filepath: {}", p_path.string());
			return nullptr;
		}
